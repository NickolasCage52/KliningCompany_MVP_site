...
function BusinessMegaMenu({ pathname }: { pathname: string }) {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute left-0 top-full z-40 w-max max-w-[calc(100vw-2rem)] xl:max-w-[48rem] pt-2 transition-all duration-200',
          open
            ? 'pointer-events-auto translate-y-0 opacity-100'
            : 'pointer-events-none -translate-y-1 opacity-0'
        )}
...
export function MainNav() {
  ...
  return (
    <nav
      className="relative hidden xl:flex w-full min-w-0 flex-1 flex-nowrap items-center justify-end gap-2 overflow-x-auto"
