...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full overflow-x-clip border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 lg:fixed lg:left-0 lg:right-0">
      <Topbar />
      ...
      <Container className="header-container w-full min-w-0 max-w-full 2xl:max-w-[1760px] px-4 sm:px-6 lg:px-8">
        <div className="grid h-20 min-h-20 w-full min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-4 sm:h-[6rem] sm:min-h-[6rem] sm:gap-6 xl:gap-8">
          {/* Логотип */}
          <Link
            href="/"
          ...
          >
            <Image
              ...
              width={340}
              height={102}
              className="h-12 w-auto max-w-[180px] object-contain object-left sm:h-14 sm:max-w-[220px] xl:h-[5rem] xl:max-w-none"
              priority
            />
          </Link>

          {/* Навигация (десктоп) и бургер (мобильный) */}
          <div className="flex min-w-0 flex-1 items-center justify-end overflow-hidden xl:min-w-0">
            <MainNav />
            <button
              ref={burgerRef}
              type="button"
